...
export class MailchimpService {
    ...
    try {
      const response = await this.transactionalClient.messages.send({
        ...
        message: {
          ...
          subject: 'Verify your email address',
          to: [{ email, type: 'to' }],
          html: `<p>Please verify your email by clicking the following link:</p></br>${verificationUrl}`,
        },
      });
      return response;
    } catch (error) {
  ...
  async sendPasswordResetEmail(email: string, token: string): Promise<void> {
    const resetUrl = `http://your-domain.com/reset-password?token=${token}`;
    ...
    try {
      await await this.transactionalClient.messages.send({ message });
    } catch (error) {
